...
BEGIN
    ...
    monthlyPayment := principalAmount * (monthlyInterestRate * POWER(1 + monthlyInterestRate, totalPayments)) / (POWER(1 + monthlyInterestRate, totalPayments) - 1);
...
DECLARE
    v_customerAge NUMBER;
    v_monthlyPayment NUMBER;
    v_isSufficientBalance BOOLEAN;
BEGIN
    -- Test CalculateAge function
    v_customerAge := CalculateAge(TO_DATE('1980-01-01', 'YYYY-MM-DD'));
    DBMS_OUTPUT.PUT_LINE('Age: ' || v_customerAge);

    -- Test CalculateMonthlyInstallment function
    v_monthlyPayment := CalculateMonthlyInstallment(10000, 5, 10);
    DBMS_OUTPUT.PUT_LINE('Monthly Installment: ' || v_monthlyPayment);

    -- Test HasSufficientBalance function
    v_isSufficientBalance := HasSufficientBalance(1, 500);
    IF v_isSufficientBalance THEN
