...
BEGIN
    FOR account_rec IN account_cursor LOOP
        IF account_rec.Balance > 5000 THEN
            ...
            DBMS_OUTPUT.PUT_LINE('CustomerID: ' || account_rec.CustomerID || ' promoted to VIP!');
        ELSE
            UPDATE Customers
            SET IsVIP = 'N'
            WHERE CustomerID = account_rec.CustomerID;
            DBMS_OUTPUT.PUT_LINE('CustomerID: ' || account_rec.CustomerID || ' demoted from VIP!');
        END IF;
    END LOOP;
END PromoteToVIP;
/
...
-- Call the procedures
BEGIN
    DBMS_OUTPUT.PUT_LINE('SCENARIO - 1: Apply discount to interest rate for customers above age 60');
